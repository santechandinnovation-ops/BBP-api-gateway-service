...
async def test_user_registration():
    async with httpx.AsyncClient() as client:
        ...
        response = await client.post(
            f"{API_GATEWAY_URL}/api/users/register",
            json=user_data
        )
        print(f"\nUser Registration: {response.status_code}")
        print(response.json())

async def test_paths_list():
    async with httpx.AsyncClient() as client:
        response = await client.get(f"{API_GATEWAY_URL}/api/paths")
        print(f"\nPaths List: {response.status_code}")
        print(response.json())
